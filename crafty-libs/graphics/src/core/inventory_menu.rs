        ...
        if let Some(carried_item) = self.carried_item {
                ...
                for col in 0..INVENTORY_NCOLS {
                    if self.inventory_slots[row][col].is_in(&self.cursor_pos) {
                        if row == 0 {
                            ...
                            if self
                                .items
                                .put_inventory_item((row - 1) * INVENTORY_NCOLS + col, carried_item)
                            {
                                self.carried_item = None;
                            }
                        }
                ...
                for col in 0..3 {
                    if self.crafting_slots[row][col].is_in(&self.cursor_pos) {
                        if let None = self.crafting_items[row][col] {
                            std::mem::swap(
                                &mut self.crafting_items[row][col],
                    ...
                    if self.inventory_slots[row][col].is_in(&self.cursor_pos) {
                        ...
                        if row == 0 {
                            ...
                            if let Some(block) = self
                                .items
                                .take_inventory_item((row - 1) * INVENTORY_NCOLS + col)
                            {
                                self.carried_item = Some(block);
                            }
                        }
            ...
            if self.crafting_output_slot.is_in(&self.cursor_pos) {
                if let Some(_) = self.crafting_output_items {
                    std::mem::swap(&mut self.crafting_output_items, &mut self.carried_item);
        ...
        // inventory slots
        {
                ...
                for col in 0..INVENTORY_NCOLS {
                    ...
                    let slot = InventorySlot::new(
                        InventoryPosition::new(
                            margin + col as f32 * (item_size + margin),
                    ...
                    self.rects.append(&mut slot.rects(
                        &self.ui_rect,
                        item,
                        slot.is_in(&self.cursor_pos),
                    ));
                }
            }
            ...
            for row in 0..3 {
                for col in 0..3 {
                    let slot = InventorySlot::new(
                        InventoryPosition::new(
                            craftx + col as f32 * (item_size + margin),
